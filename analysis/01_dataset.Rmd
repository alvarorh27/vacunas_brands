...
grouping_vaccines <- function(vaccines) {
  if (all(vaccines == "Pfizer/BioNTech", na.rm = TRUE)) {
    ...
    return("CoronaVac (Sinovac Biotech)")
...
# Adding group_vaccines and location_vaccines 
vacunas3 <- vacunas3 %>%
  rowwise() %>%
  mutate(group_vaccines = grouping_vaccines(c(shot_1, shot_2, shot_3)),
         location_vaccines = eu_and_noneu_vaccines(c(shot_1, shot_2, shot_3))) %>%
  mutate(
    group_vaccines = as.factor(group_vaccines),
    location_vaccines = as.factor(location_vaccines)
  ) %>%
  select(
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(across(shot_1:location_vaccines, ~ if_else(is.na(.x), "None", .x))) %>%
  mutate(across(shot_1:location_vaccines, ~ as.factor(.x))) %>%
  mutate(across(shot_1:location_vaccines, ~ relevel(.x, ref = "None"))) %>%
  mutate(
