...
# Adding group_vaccines and european_vaccines 
vacunas3 <- vacunas3 %>%
  rowwise() %>%
  ...
  mutate(
    ...
    european_vaccines = as.factor(european_vaccines)
  ) %>%
  select(
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(across(shot_1:european_vaccines, ~ if_else(is.na(.x), "None", .x))) %>%
  mutate(across(shot_1:european_vaccines, ~ as.factor(.x))) %>%
  mutate(across(shot_1:european_vaccines, ~ relevel(.x, ref = "None"))) %>%
  mutate(
