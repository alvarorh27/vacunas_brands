...
vacunas3 <- setDT(vacunas2) %>% 
  ...
  select(NUHSA_ENCRIPTADO, orden_infeccion, n_prev_infecc, n_vacunas_covid, shot_1:shot_3, all_three_shots, ultima_vac_infecc_meses_quant, ultima_inmun_infecc_meses_quant, n_exposi) %>%
  as.data.frame()
...
# Adding variables applying previous functions  
vacunas3 <- vacunas3 %>%
  rowwise() %>%
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(
    ...
    ultima_inmun_infecc_dias = if_else(
      n_vacunas_covid == 0 & !is.na(prev_infect_date),
      as.numeric(infect_covid_first_date - prev_infect_date),
      999 # problema en la resta q no contempla el NA
    ),
  ...
  select(-ultima_inmun_infecc_dias)
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(
    ...
    ultima_inmun_infecc_bin = relevel(ultima_inmun_infecc_bin, ref = "Less than one month"),
...
estudio_vacunas_t_final_selected$ultima_inmun_infecc_bin <- factor(
  ...
  levels = c(NA, "Less than one month", "One to three months", "Six months or more", "Three to six months" ),
  labels = c("Ninguna", "Less than one month", "One to three months", "Six months or more", "Three to six months"),
