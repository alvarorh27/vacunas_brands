...
# TODO apply exclusion criteria following the reasoned order of dataset loading!
# TODO MODIFY THE STUDY PERIOD. WE ONLY HAVE PCR TESTS AND VACCINES UP TO MARCH 31, 2022
...
saveRDS(
  poblacion_diana3, 
  file = "~/projects/vacunas_brands_2025/Rfiles/poblacion_diana3.rds"
)
...
saveRDS(
  infect_covid5, 
  file = "~/projects/vacunas_brands_2025/Rfiles/infect_covid5.rds"
)
...
saveRDS(
  pupa2, 
  file = "~/projects/vacunas_brands_2025/Rfiles/pupa2.rds"
)
...
saveRDS(
  episodios_cmbd, 
  file = "~/projects/vacunas_brands_2025/Rfiles/episodios_cmbd.rds"
)
...
## Remove "COVID-19" from names of vaccines
vacunas_raw <- vroom("/opt/datos_compartidos/vacunas_data/total/ENC_VACUNAS_VACUNAS_TODO.txt") %>%
  filter(grepl("COVID-19", NOM_VACUNA, fixed = TRUE)) %>%
  mutate(NOM_VACUNA = gsub("COVID-19 ", "", NOM_VACUNA, fixed = TRUE)) %>%
  as.data.frame() %>% 
  distinct()

# Creating vaccine groups vectors
...
vacunas_inprogress <- setDT(vacunas_raw) %>% 
  lazy_dt(immutable = TRUE) %>% 
  filter(NOM_VACUNA %in% eu_covid_vaccines) %>% 
...
vacunas3 <- setDT(vacunas2) %>% 
  ...
  summarise(
    n_vacunas_covid = n(),
    ultima_vac_infecc_dias = unique(ultima_vac_infecc_dias),
  ...
  mutate(all_three_shots = as.factor(paste(shot_1, shot_2, shot_3, sep = ", "))) %>%
  mutate(n_exposi = n_vacunas_covid + n_prev_infecc) %>% 
  select(NUHSA_ENCRIPTADO, orden_infeccion, n_prev_infecc, n_vacunas_covid, shot_1:shot_3, all_three_shots, ultima_vac_infecc_meses_quant, ultima_inmun_infecc_meses_quant, ultima_inmun_infecc_dias, ultima_vac_infecc_dias, n_exposi) %>%
  as.data.frame()

# Defining function eu_vaccine categories. Group with european vaccines (single-branded shots)
function_eu_vaccines <- function(vaccines) {
...
table_vac_1shot <- table(vacunas3[vacunas3$n_vacunas_covid == 1, ]$shot_1)
df_table_vac_1shot <- as.data.frame(table_vac_1shot) %>% 
  filter(Freq > 0)

# Defining function 1shot_vaccines to categorize patients with only 1 shot
function_1shot_vaccines <- function(df) {
  df %>%
    mutate(
      vac_1shot = case_when(
        n_vacunas_covid == 1 & shot_1 == "Pfizer/BioNTech" ~ "Pfizer/BioNTech",
        n_vacunas_covid == 1 & shot_1 == "Moderna" ~ "Moderna",
        n_vacunas_covid == 1 & shot_1 == "AstraZeneca" ~ "AstraZeneca",
        n_vacunas_covid == 1 & shot_1 == "Janssen" ~ "Janssen",
        n_vacunas_covid == 1 & shot_1 == "Novavax" ~ "Novavax",
        TRUE ~ "Other" #other combinations of 2 or 3 shots 
      ),
      vac_1shot = factor(vac_1shot)
    )
}

#Look at 2-shot combination
table_vac_2shot <- table(vacunas3[vacunas3$n_vacunas_covid == 2, ]$all_three_shots)
df_table_vac_2shot <- as.data.frame(table_vac_2shot) %>% 
  filter(Freq > 0)

# Defining function 2shot_vaccines to categorize patients with 2 shot (2-shot combinations with <10 frequency 
# have been considered as other)
function_2shot_vaccines <- function(df) {
  df %>%
    mutate(
      vac_2shot = case_when(
        all_three_shots == "AstraZeneca, AstraZeneca, NA" ~ "AstraZeneca+AstraZeneca",
        all_three_shots == "Moderna, Moderna, NA" ~ "Moderna+Moderna",
        all_three_shots == "Pfizer/BioNTech, Pfizer/BioNTech, NA" ~ "Pfizer/BioNTech+Pfizer/BioNTech",
        all_three_shots == "Janssen, Pfizer/BioNTech, NA" ~ "Janssen+Pfizer/BioNTech",
        all_three_shots == "Janssen, Moderna, NA" ~ "Janssen+Moderna",
        all_three_shots == "AstraZeneca, Pfizer/BioNTech, NA" ~ "AstraZeneca+Pfizer/BioNTech",
        all_three_shots == "Pfizer/BioNTech, Moderna, NA" ~ "Pfizer/BioNTech+Moderna",
        all_three_shots == "Moderna, Pfizer/BioNTech, NA" ~ "Moderna+Pfizer/BioNTech",
        all_three_shots == "AstraZeneca, Moderna, NA" ~ "AstraZeneca+Moderna",
        TRUE ~ "Other" #other combinations of shots 
      ),
      vac_2shot = factor(vac_2shot)
    )
}

#Look at 3-shot combination
table_vac_3shot <- table(vacunas3[vacunas3$n_vacunas_covid == 3, ]$all_three_shots)
df_table_vac_3shot <- as.data.frame(table_vac_3shot) %>% 
  filter(Freq > 0)

# Defining function 3shot_vaccines to categorize patients with 3 shot (3-shot combinations with <10 frequency 
# have been considered as other)
function_3shot_vaccines <- function(df) {
  df %>%
    mutate(
      vac_3shot = case_when(
        all_three_shots == "Pfizer/BioNTech, Pfizer/BioNTech, Pfizer/BioNTech" ~ "Pfizer/BioNTech+Pfizer/BioNTech+Pfizer/BioNTech",
        all_three_shots == "AstraZeneca, AstraZeneca, Moderna" ~ "AstraZeneca+AstraZeneca+Moderna",
        all_three_shots == "AstraZeneca, AstraZeneca, Pfizer/BioNTech" ~ "AstraZeneca+AstraZeneca+Pfizer/BioNTech",
        all_three_shots == "Pfizer/BioNTech, Pfizer/BioNTech, Moderna" ~ "Pfizer/BioNTech+Pfizer/BioNTech+Moderna",
        all_three_shots == "Moderna, Moderna, Moderna" ~ "Moderna+Moderna+Moderna",
        all_three_shots == "Moderna, Moderna, Pfizer/BioNTech" ~ "Moderna+Moderna+Pfizer/BioNTech",
        all_three_shots == "AstraZeneca, Pfizer/BioNTech, Pfizer/BioNTech" ~ "AstraZeneca+Pfizer/BioNTech+Pfizer/BioNTech",
        all_three_shots == "AstraZeneca, Pfizer/BioNTech, Moderna" ~ "AstraZeneca+Pfizer/BioNTech+Moderna",
        all_three_shots == "Moderna, Pfizer/BioNTech, Pfizer/BioNTech" ~ "Moderna+Pfizer/BioNTech+Pfizer/BioNTech",
        TRUE ~ "Other" #other combinations of shots
      ),
      vac_3shot = factor(vac_3shot)
    )
}

# Defining function eu_and_noneu_vaccines to categorize european and non-european vaccines
function_eu_and_noneu_vaccines <- function(vaccines) {
...
# Defining rna_vaccine categories. Group with european RNA, non-RNA and mix of ARN and non-RNA vaccines
function_rna_vaccines <- function(vaccines) {
  if (length(unique(vaccines[vaccines %in% eu_rna_vaccines])) > 1) {
  ...
  } else if (any(vaccines %in% eu_rna_vaccines, na.rm = TRUE) &
             any(vaccines %in% eu_nonrna_vaccines, na.rm = TRUE)) {
...
# Defining combined_vaccines categories. 10 most common combinations (excluding combinations composed only of the same brand, as this appears in eu_vaccines variable)
function_combined_vaccines <- function(vaccines) {
  if (vaccines == "AstraZeneca, AstraZeneca, Moderna") {
    return("AstraZeneca+AstraZeneca+Moderna")
  } else if (vaccines == "AstraZeneca, AstraZeneca, Pfizer/BioNTech") {
    return("AstraZeneca+AstraZeneca+Pfizer/BioNTech")
  } else if (vaccines == "Pfizer/BioNTech, Pfizer/BioNTech, Moderna") {
    return("Pfizer/BioNTech+Pfizer/BioNTech+Moderna")
  } else if (vaccines == "Janssen, Pfizer/BioNTech, NA") {
    return("Janssen+Pfizer/BioNTech")
  } else if (vaccines == "Moderna, Moderna, Pfizer/BioNTech") {
    return("Moderna+Moderna+Pfizer/BioNTech")
  } else if (vaccines == "Janssen, Moderna, NA") {
    return("Janssen+Moderna")
  } else if (vaccines == "AstraZeneca, Pfizer/BioNTech, NA") {
    return("AstraZeneca+Pfizer/BioNTech")
  } else if (vaccines == "Pfizer/BioNTech, Moderna, NA") {
    return("Pfizer/BioNTech+Moderna")
  } else if (vaccines == "Moderna, Pfizer/BioNTech, NA") {
    return("Moderna+Pfizer/BioNTech")
  } else if (vaccines == "AstraZeneca, Pfizer/BioNTech, Pfizer/BioNTech") {
    return("AstraZeneca+Pfizer/BioNTech+Pfizer/BioNTech")
  } else if (vaccines == "Pfizer/BioNTech, Pfizer/BioNTech, NA") {
    return("Pfizer/BioNTech+Pfizer/BioNTech")
  } else if (vaccines == "Pfizer/BioNTech, Pfizer/BioNTech, Pfizer/BioNTech") {
    return("Pfizer/BioNTech+Pfizer/BioNTech+Pfizer/BioNTech")
  } else if (vaccines == "Moderna, Moderna, NA") {
    return("Moderna+Moderna")
  } else if (vaccines == "Moderna, Moderna, Moderna") {
    return("Moderna+Moderna+Moderna")
  } else if (vaccines == "AstraZeneca, AstraZeneca, NA") {
    return("AstraZeneca+AstraZeneca")
  } else if (vaccines == "Pfizer/BioNTech, NA, NA") {
    return("Pfizer/BioNTech")
  } else if (vaccines == "Janssen, NA, NA") {
    return("Janssen")
  } else if (vaccines == "AstraZeneca, NA, NA") {
    return("AstraZeneca")
  } else if (vaccines == "Moderna, NA, NA") {
    return("Moderna")
  }  else {
    return("Other")
  }
...
# Adding variables applying previous vaccine-grouping functions  
vacunas3 <- vacunas3 %>%
  function_1shot_vaccines() %>%  
  function_2shot_vaccines() %>%  
  function_3shot_vaccines() %>%
  rowwise() %>%
  mutate(eu_vaccines = function_eu_vaccines(c(shot_1, shot_2, shot_3)),
         rna_vaccines = function_rna_vaccines(c(shot_1, shot_2, shot_3)),
         combined_vaccines = function_combined_vaccines(all_three_shots), 
         eu_and_noneu_vaccines = function_eu_and_noneu_vaccines(c(shot_1, shot_2, shot_3))) %>%
  mutate(
    eu_vaccines = as.factor(eu_vaccines),
    rna_vaccines = as.factor(rna_vaccines),
    combined_vaccines = as.factor(combined_vaccines), 
    eu_and_noneu_vaccines = as.factor(eu_and_noneu_vaccines)
  ) %>%
  select(
    NUHSA_ENCRIPTADO:shot_3,
    all_three_shots,
    eu_vaccines, eu_and_noneu_vaccines, rna_vaccines, combined_vaccines,
    vac_1shot, vac_2shot, vac_3shot,
    n_prev_infecc:n_exposi
  ) %>%
  arrange(NUHSA_ENCRIPTADO)

# Create table to see most common shot combination
table_3shots <- table(vacunas3$all_three_shots)
tabla_ordenada <- table_3shots[order(-table_3shots)]
write.table(tabla_ordenada, file = "~/projects/vacunas_brands_2025/figures/combined_vaccines.csv", sep = ":", col.names = TRUE, row.names = TRUE, quote = FALSE)
print(tabla_ordenada)

saveRDS(
  vacunas3, 
  file = "~/projects/vacunas_brands_2025/Rfiles/vacunas3.rds"
)
...
episodios_cmbd <- readRDS("~/projects/vacunas_brands_2025/Rfiles/episodios_cmbd.rds")
infect_covid5 <- readRDS("~/projects/vacunas_brands_2025/Rfiles/infect_covid5.rds")
poblacion_diana3 <- readRDS("~/projects/vacunas_brands_2025/Rfiles/poblacion_diana3.rds")
pupa2 <- readRDS("~/projects/vacunas_brands_2025/Rfiles/pupa2.rds")
vacunas3 <- readRDS("~/projects/vacunas_brands_2025/Rfiles/vacunas3.rds")
...
# TODO explain why patients who had not received any dose at the time of infection have an NA value in
# inmun_mas6meses_bin & inmun_meses_quant

# TODO I understand that this analysis should be restricted to vaccinated patients. You cannot assign an
# "arbitrary" value to unvaccinated patients, as that would distort the measurement of the effect of
# being vaccinated among those who were vaccinated.
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(
    ...
    ultima_vac_infecc_bin = case_when(
      n_vacunas_covid != 0 & ultima_vac_infecc_dias < 31 ~ "14-30",
      n_vacunas_covid != 0 & ultima_vac_infecc_dias < 91 ~ "31-90",
      n_vacunas_covid != 0 & ultima_vac_infecc_dias < 181 ~ "91-180",
      n_vacunas_covid != 0 & ultima_vac_infecc_dias > 180 ~ ">180",
      TRUE ~ NA_character_,
    ),
...
estudio_vacunas_t_final_selected %<>%
  mutate(n_vacunas_covid = as.factor(
    case_when(
  ...
  mutate(vaccine_bin = if_else(n_vacunas_covid=="Ninguna", "No", "Sí")) %>%
  mutate(vaccine_bin = as.factor(vaccine_bin)) %>%
  mutate(vaccine_bin = relevel(vaccine_bin, ref = "No")) %>%
  mutate(across(shot_1:vac_3shot, ~ if_else(is.na(.x), "None", .x))) %>%
  mutate(across(shot_1:vac_3shot, ~ as.factor(.x))) %>%
  mutate(across(shot_1:vac_3shot, ~ relevel(.x, ref = "None"))) %>%
  mutate(
    n_vacunas_covid = relevel(n_vacunas_covid, ref = "Ninguna"),
    n_prev_infecc = as.factor(
      case_when(
        n_prev_infecc == 2 ~ "Dos",
        n_prev_infecc == 1 ~ "Uno",
        n_prev_infecc == 0 ~ "Ninguna"
      )
    ),
    ...
    ultima_inmun_infecc_bin2 = case_when(
      # (n_vacunas_covid != 0 |  !is.na(prev_infect_date)) & ultima_inmun_infecc_dias < 14 ~ "0-14",
      (n_vacunas_covid != 0 |  !is.na(prev_infect_date)) & ultima_inmun_infecc_dias < 31 ~ "14-30",
      (n_vacunas_covid != 0 |  !is.na(prev_infect_date)) & ultima_inmun_infecc_dias < 91 ~ "31-90",
      (n_vacunas_covid != 0 |  !is.na(prev_infect_date)) & ultima_inmun_infecc_dias < 181 ~ "91-180",
      (n_vacunas_covid != 0 |  !is.na(prev_infect_date)) & ultima_inmun_infecc_dias > 180 ~ ">180",
      TRUE ~ NA
    ),
    ultima_vac_infecc_bin = as.factor(ultima_vac_infecc_bin),
    ultima_vac_infecc_bin = relevel(ultima_vac_infecc_bin, ref = "14-30"),
    ...
    ultima_inmun_infecc_bin2 = as.factor(ultima_inmun_infecc_bin2),
    ultima_inmun_infecc_bin2 = relevel(ultima_inmun_infecc_bin2, ref = "14-30"),
    ...
    clinical_event = if_else(hosp_stay_bin=="No" & icu_stay_bin=="No" & mortality30d=="No", "No", "Sí"),
    clinical_event = as.factor(clinical_event),
    clinical_event = relevel(clinical_event, ref = "No"), 
    clinical_event_nodeath = if_else(hosp_stay_bin=="No" & icu_stay_bin=="No", "No", "Sí"),
    clinical_event_nodeath = as.factor(clinical_event_nodeath),
    clinical_event_nodeath = relevel(clinical_event_nodeath, ref = "No")
  )
...
estudio_vacunas_t_final_selected$n_vacunas_covid <- factor(estudio_vacunas_t_final_selected$n_vacunas_covid, levels = c("Ninguna", "Una", "Dos", "Tres"))
estudio_vacunas_t_final_selected$n_prev_infecc <- factor(estudio_vacunas_t_final_selected$n_prev_infecc, levels = c("Ninguna", "Uno", "Dos"))
estudio_vacunas_t_final_selected$n_exposi <- factor(estudio_vacunas_t_final_selected$n_exposi, levels = c("Ninguna", "Una", "Dos", "Tres", "Cuatro", "Cinco"))
estudio_vacunas_t_final_selected$orden_infeccion <- factor(estudio_vacunas_t_final_selected$orden_infeccion, levels = unique(estudio_vacunas_t_final_selected$orden_infeccion))
estudio_vacunas_t_final_selected$age_fct_60years <- factor(estudio_vacunas_t_final_selected$age_fct_60years, levels = c("Menor de 60 años", "60 años o más"))
estudio_vacunas_t_final_selected$variant_period <- as.factor(estudio_vacunas_t_final_selected$variant_period)
estudio_vacunas_t_final_selected$variant_period <- factor(estudio_vacunas_t_final_selected$variant_period, levels = c("Before α", "α", "α/Δ transition", "Δ", "Δ/Ό transition", "Ό"))
estudio_vacunas_t_final_selected$variant_period <- relevel(estudio_vacunas_t_final_selected$variant_period, ref = "Before α")

estudio_vacunas_t_final_selected$ultima_vac_infecc_bin <- factor(
  estudio_vacunas_t_final_selected$ultima_vac_infecc_bin,
  levels = c(NA, "14-30", "31-90", "91-180", ">180"),
  labels = c("None", "14-30", "31-90", "91-180", ">180"),
...
estudio_vacunas_t_final_selected$ultima_inmun_infecc_bin2 <- factor(
  estudio_vacunas_t_final_selected$ultima_inmun_infecc_bin2,
  levels = c(NA, "14-30", "31-90", "91-180", ">180"),
  labels = c("None", "14-30", "31-90", "91-180", ">180"),
  exclude = NULL  # This ensures that <NA> is treated as a regular level
)
...
estudio_vacunas_t_final_selected %<>%
  mutate(ultima_vac_infecc_bin = relevel(ultima_vac_infecc_bin, ref = "14-30"))

estudio_vacunas_t_final_selected %<>%
  mutate(ultima_inmun_infecc_bin = relevel(ultima_inmun_infecc_bin, ref = "14-30"))

estudio_vacunas_t_final_selected %<>%
  mutate(ultima_inmun_infecc_bin2 = relevel(ultima_inmun_infecc_bin2, ref = "14-30"))
...
estudio_vacunas_t_final_selected$eu_vaccines <- factor(estudio_vacunas_t_final_selected$eu_vaccines, levels = c("None", "Pfizer/BioNTech", "Moderna", "AstraZeneca", "Janssen", "Other"))
estudio_vacunas_t_final_selected$eu_vaccines <- relevel(estudio_vacunas_t_final_selected$eu_vaccines, ref = "None")

estudio_vacunas_t_final_selected$eu_and_noneu_vaccines <- factor(estudio_vacunas_t_final_selected$eu_and_noneu_vaccines, levels = c("None", "EU Vaccines", "Non-EU Vaccines", "EU and non-EU Vaccines"))
estudio_vacunas_t_final_selected$eu_and_noneu_vaccines <- relevel(estudio_vacunas_t_final_selected$eu_and_noneu_vaccines, ref = "None")

estudio_vacunas_t_final_selected$rna_vaccines <- factor(estudio_vacunas_t_final_selected$rna_vaccines, levels = c("None", "Mix of EU RNA vaccines", "Mix of EU non-RNA vaccines", "Mix of EU RNA and non-RNA vaccines"))
estudio_vacunas_t_final_selected$rna_vaccines <- relevel(estudio_vacunas_t_final_selected$rna_vaccines, ref = "None")

estudio_vacunas_t_final_selected$combined_vaccines <- factor(estudio_vacunas_t_final_selected$combined_vaccines, levels = c("None", "Pfizer/BioNTech+Pfizer/BioNTech", "Pfizer/BioNTech+Pfizer/BioNTech+Pfizer/BioNTech", "Moderna+Moderna", "AstraZeneca+AstraZeneca", "Pfizer/BioNTech", "Janssen", "AstraZeneca", "Moderna", "AstraZeneca+AstraZeneca+Moderna", "AstraZeneca+AstraZeneca+Pfizer/BioNTech", "Pfizer/BioNTech+Pfizer/BioNTech+Moderna", "Moderna+Moderna+Moderna", "Janssen+Pfizer/BioNTech", "Moderna+Moderna+Pfizer/BioNTech","Janssen+Moderna",  "AstraZeneca+Pfizer/BioNTech", "Other", "Pfizer/BioNTech+Moderna", "Moderna+Pfizer/BioNTech", "AstraZeneca+Pfizer/BioNTech+Pfizer/BioNTech"))
estudio_vacunas_t_final_selected$combined_vaccines <- relevel(estudio_vacunas_t_final_selected$combined_vaccines, ref = "None")

estudio_vacunas_t_final_selected$vac_1shot <- relevel(estudio_vacunas_t_final_selected$vac_1shot, ref = "None")
estudio_vacunas_t_final_selected$vac_2shot <- relevel(estudio_vacunas_t_final_selected$vac_2shot, ref = "None")
estudio_vacunas_t_final_selected$vac_3shot <- relevel(estudio_vacunas_t_final_selected$vac_3shot, ref = "None")
...
saveRDS(
  estudio_vacunas_t_final_selected, 
  file = "~/projects/vacunas_brands_2025/Rfiles/final_dataset_brands.rds"
)

saveRDS(
  variants_week_spain_labels,
  file = "~/projects/vacunas_brands_2025/Rfiles/variants_week_spain_labels.rds",
)
```
