...
# Defining eu_vaccine categories. Group with european vaccines (single-branded shots)
...
# Adding eu_vaccines and rna_vaccines 
vacunas3 <- vacunas3 %>%
  rowwise() %>%
...
saveRDS(
  vacunas3, 
  file = "~/projects/vacunas_brands/data/vacunas3.rds"
)
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(vaccine_bin = as.factor(if_else(n_vacunas_covid=="Ninguna", "No", "Sí"))) %>%
  mutate(vaccine_bin = relevel(vaccine_bin, ref = "No")) %>%
  mutate(across(shot_1:shot_3, ~ if_else(is.na(.x), "None", .x))) %>%
  mutate(across(shot_1:shot_3, ~ as.factor(.x))) %>%
  mutate(across(shot_1:shot_3, ~ relevel(.x, ref = "None"))) %>%
  mutate(across(eu_vaccines:combined_vaccines, ~ relevel(.x, ref = "Other"))) %>%
  mutate(
    ...
    clinical_event = as.factor(ifelse(hosp_stay_bin=="Sí" | icu_stay_bin=="Sí" | mortality30d=="Sí", "Sí", "No")),
...
saveRDS(
  estudio_vacunas_t_final_selected, 
  file = "~/projects/vacunas_brands/data/estudio_vacunasbrands_t_final_selected_manuscript.rds"
)

saveRDS(
  variants_week_spain_labels,
  file = "~/projects/vacunas_brands/data/variants_week_spain_labels.rds",
)
```
